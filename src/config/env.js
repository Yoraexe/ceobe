...
function loadEnv() {
    ...
    const getOptional = (key) => process.env[key] || '';
    const config = {
        ...
        CEOBE_SANDBOX: (['docker', 'none'].includes(process.env.CEOBE_SANDBOX) ? process.env.CEOBE_SANDBOX : 'docker'), // Fix M-03: Secure default
...
function getGatewayUrl(provider) {
    if (!exports.env.CLOUDFLARE_ACCOUNT_ID || !exports.env.CLOUDFLARE_GATEWAY_ID) {
        return '';
    }
    ...
    return `https://gateway.ai.cloudflare.com/v1/${exports.env.CLOUDFLARE_ACCOUNT_ID}/${exports.env.CLOUDFLARE_GATEWAY_ID}/${slug}`;
}
