  ...
  private getClient(): unknown {
    if (!this.client) {
      if (!env.GEMINI_API_KEY) {
        throw new Error('[GeminiEmbeddingAdapter] GEMINI_API_KEY environment variable is not set.');
      }
...
class OpenAICompatibleEmbeddingAdapter implements IEmbeddingAdapter {
  ...
  private async getClient(): Promise<unknown> {
    if (!this.client) {
      if (!cachedOpenAIModule) {
        cachedOpenAIModule = (await import('openai')).default;
      }
      const OpenAI = cachedOpenAIModule;
  ...
  async getEmbedding(text: string): Promise<number[]> {
    ...
    if (vec.length === 0) {
      throw new Error(`[EmbeddingAdapter] Empty embedding vector returned by ${this.name} API.`);
    }
    return vec;
  }
}
