...
import { getActiveSession, getSessionMode, setSessionMode } from '../sessionManager';
import { executionContext } from '../../utils/context';
import { MessageQueue } from '../messageQueue';
import { getActiveMode } from '../../utils/modeManager';
...
export async function handleStatusCommand(bot: TelegramBot, chatId: number, queue: MessageQueue) {
  ...
  const sessionMode = getSessionMode(chatId) || getActiveMode();
  await executionContext.run({ projectPath: pPath, mode: sessionMode }, async () => {
...
export async function handleModeCommand(bot: TelegramBot, chatId: number, text: string) {
  let newMode = getSessionMode(chatId) || getActiveMode();
